...
import { QuoteCreatedByRequiredError, QuoteEntity } from "@domain";
import {
  QuoteDtoMapper,
  CreateQuoteCommand,
  IQuoteRepository,
...
export class CreateQuoteCommandHandler implements CommandHandler<
  CreateQuoteCommand,
  unknown
> {
  ...
  async handle(command: CreateQuoteCommand): Promise<unknown> {
    return command.executeWithTracing(async (cmd) => {
      ...
      const props = QuoteDtoMapper.fromDto(cmd.data, cmd.createdBy);
