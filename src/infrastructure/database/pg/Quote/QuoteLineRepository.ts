...
export class QuoteLineRepository {
  ...
  async findByQuoteId(quoteId: string): Promise<QuoteLineRecord[]> {
    const result = await this.query(
      this.pool,
      `SELECT id, quote_id, type, product_id, quantity, unit_price, product_name, position
       FROM ${this.tableName}
       WHERE quote_id = $1
  ...
  async insertMany(records: QuoteLineRecord[], client?: PoolClient): Promise<void> {
    ...
    for (const r of records) {
      await this.query(
        queryable,
        `INSERT INTO ${this.tableName} (id, quote_id, type, product_id, quantity, unit_price, product_name, position)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [r.id, r.quote_id, r.type ?? null, r.product_id, r.quantity, r.unit_price, r.product_name ?? null, r.position]
      );
    }
  }
