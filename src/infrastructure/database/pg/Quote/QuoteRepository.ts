...
export class QuoteRepository extends BaseRepository<QuoteRecord> {
  ...
  async findById(id: number): Promise<QuoteRecordWithLines | null> {
    const quote = await this.getRecord(String(id));
  ...
  async findLastRegistry(): Promise<QuoteRecordWithLines | null> {
    ...
    const lines = await this.lineRepo.findByQuoteId(quote.id);
    return { ...quote, lines };
  }
  ...
  async delete(id: number): Promise<void> {
    await this.deleteRecord(String(id));
  }
}
