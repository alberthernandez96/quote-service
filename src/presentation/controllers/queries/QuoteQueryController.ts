...
import { QuoteResponseDTO, QuoteListResponseDTO, getQuoteListQuerySchema } from '@albertoficial/api-contracts';
...
export class QuoteQueryController {
  ...
  async get(req: Request, res: Response<QuoteResponseDTO>): Promise<void> {
    try {
      ...
      const result = await this.queryBus.execute<QuoteResponseDTO>(
        new GetQuoteQuery(Number(id), undefined, req.headers['x-correlation-id'] as string)
      );
  ...
  async getLastRegistry(req: Request, res: Response<QuoteResponseDTO>): Promise<void> {
    try {
      const result = await this.queryBus.execute<QuoteResponseDTO>(
